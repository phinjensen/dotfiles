...
beautiful.init(gears.filesystem.get_xdg_config_home() .. "awesome/themes/seoul256/theme.lua")
...
function brightness.update()
    awful.spawn.easy_async_with_shell("light -G", function(out)
        brightness.markup = markup.small(string.format("BRI: %3d%%", string.sub(out, 0, string.find(out, "%.")-1)))
    end)
end
...
local mycpu = lain.widget.cpu {
    settings = function()
        widget:set_markup(markup.small(string.format("CPU: %3d%%", cpu_now.usage)))
    end
}

local mymem = lain.widget.mem {
    settings = function()
        widget:set_markup(markup.small(string.format("RAM: %3d%%", mem_now.perc)))
    end
}
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift"   }, "q", function () awful.spawn.easy_async("qdbus org.kde.ksmserver /KSMServer org.kde.KSMServerInterface.logout 0 0 -1") end,
    ...
    --    function()
    --        awful.spawn.easy_async_with_shell("light -A 5", function(stdout)
    --            brightness.update()
    --        end)
    --    end),
    --awful.key({ }, "XF86MonBrightnessDown",
    --    function()
    --        awful.spawn.easy_async_with_shell("light -U 5", function(stdout)
    --            brightness.update()
    --        end)
    --    end)
)
...
awful.rules.rules = {
    ...
    { rule_any = {
        ...
        name = {
          "Event Tester",  -- xev.
          "Android Emulator - Pixel_4a_API_30:5554",
          "QEMU",
    ...
    { rule_any = { name = { "Desktop — Plasma" }, class = { "ksplashqml" } },
